...
DeepSeek Harness 的桌宠插件：显示 DeepSeek 账户余额 + 当前会话用量/花费；deepseek 精灵表桌宠（9 态动画 + 活动触发状态机），可拖拽、点击开面板；后台会话运行时头顶悬浮进度卡。
...
- 素材：deepseek 精灵表（8×9 网格，`assets/deepseek-sprite.webp`，base64 内联进 client.js；旧版 dafeiyu 鲸鱼娘立绘保留为历史素材）
